...
| 포인트 잔액 조회 API | 오퍼월(FE)이 표시 |
| 아이템 구매 / 포인트 인출 API | 오퍼월(FE) |
| 광고 상세 화면 직접 호출 | 오퍼월(FE) |
| 네이티브 광고 목록 뷰 | `TnkOfferwallView` 로 대체 |
| 오퍼월 UI 커스터마이징 API | 오퍼월(FE)에서 개발사별로 설정. 담당자 문의 |
| 사용 활동 · 구매 분석 리포팅 | 네이티브 SDK 의 `actionCompleted` / `buyCompleted` 에 해당하는 API 는 없습니다 |
