...
sdk.getAdvertiseTotalPoint { [weak self] info in
    guard let info, info.isSuccess else { return }
    self?.button.setTitle("최대 \(info.pointAmount)P 적립 가능 (\(info.adCount)개)", for: .normal)
}

// 구매형(CPS) — 쇼핑 적립 광고
sdk.getProductTotalPoint { info in /* 동일 */ }
```

- 콜백은 **항상 메인 스레드**에서 호출됩니다 — 곧바로 UI 를 갱신해도 됩니다.
- 네트워크 오류·응답 실패 시 **`nil`** 이 전달됩니다. 사유는 [`enableLogging(_:)`](initialize.md) 로 확인하세요.

## `TotalPointInfo`

| 필드 | 타입 | 설명 |
|------|------|------|
| `pointAmount` | `Int64` | 적립 가능 포인트 총합. 없으면 `0` |
| `adCount` | `Int` | 적립 가능 광고 수. 없으면 `0` |
| `isSuccess` | `Bool` | 서버가 성공으로 응답했는지 |
| `retCode` | `Int` | 응답 코드. `0` 이 성공 |
| `retMessage` | `String?` | 실패 메시지. 없으면 `nil` |
